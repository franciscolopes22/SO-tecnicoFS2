...
int tfs_init() {
    ...
    if (pthread_mutex_lock(&single_global_lock) != 0)
        return -1;
    // initialize TFS_started variable (used to disallow tfs_open calls
    // when tfs_destroy has been called)
    TFS_started = 1;
...
// waits until there's no opened files and after that, terminates TecnicoFS
int tfs_destroy_after_all_closed() {

    if (pthread_mutex_lock(&single_global_lock) != 0)
        return -1;
    
    // waits until there's no opened files
    while (opened_files != 0){
        pthread_cond_wait(&cond, &single_global_lock);
    }

    // disable possible calls to tfs_open
    TFS_started = 0;
    
    if (pthread_mutex_unlock(&single_global_lock) != 0)
        return -1;

    // successful, call tfs_destroy to terminate TecnicoFS
    tfs_destroy();
    return 0;
}
...
int tfs_open(char const *name, int flags) {
    ...
    // if tfs_destroy_all has been called, return -1    
    if (TFS_started == 0) {
        return -1;
    }

    int ret = _tfs_open_unsynchronized(name, flags);
    // if successfuly added to the opened file table, increment number of opened_files
    if (ret != -1) {
        opened_files++;
    }
    if (pthread_mutex_unlock(&single_global_lock) != 0)
...
int tfs_close(int fhandle) {
    ...
    // if successfuly removed from the opened file table, decrement number of opened_files
    if (r != -1) {
        opened_files--;
    }
    // if there's no opened_files anymore, signal the conditional variable waiting;
    if (opened_files == 0) {
